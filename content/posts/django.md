...
### 2. Do not override `save()` or use signals, create `services` to abstract away the business logic.
...
### 3. Don't use General Foreign Key (GFK)
...
### 4. Explicitly define all tables and their names
...
### 5. Create `?include=` query param when dealing with deep objects
